...
class HomeNetworkManager {
    ...
    func getLocationVefirication() -> Single<LocationVerificationModel> {
        return authProvider.rx.request(MultiTarget(HomeAPIs.getLocationVerification))
    ...
    func getAddressFromGeocoder(longitude: Double, latitude: Double) -> Single<GeocoderResponseModel> {
        ...
        return provider.rx.request(MultiTarget(HomeAPIs.getAddress(point: point)))
            .tryMap(GeocoderResponseModel.self)
    }
    
    // 사용자 위치 인증
    func patchLocationVerification(address: String) -> Single<LocationVerificationModel> {
        return authProvider.rx.request(MultiTarget(HomeAPIs.patchLocationVerification(address: address)))
            .tryMap(LocationVerificationModel.self)
    }
    
    // 홈 게시글 목록 불러오기
    func getHomeList(sortBy: String, page: Int, size: Int) -> Single<PostListResponseModel> {
        return authProvider.rx.request(MultiTarget(HomeAPIs.getHomeList(sortBy: sortBy, page: page, size: size)))
    ...
    func getHomeListByCategories(categories: [String], sortBy: String, page: Int, size: Int) -> Single<PostListResponseModel> {
        return authProvider.rx.request(MultiTarget(HomeAPIs.getHomeListByCategories(category: categories, sortBy: sortBy, page: page, size: size)))
            .tryMap(PostListResponseModel.self)
    }
    ...
    func getSearchList(keyword: String, sortBy: String, page: Int, size: Int) -> Single<PostListResponseModel> {
        return authProvider.rx.request(MultiTarget(HomeAPIs.getSearchList(keyword: keyword, sortBy: sortBy, page: page, size: size)))
            .tryMap(PostListResponseModel.self)
    }
    ...
    func getPost(id: Int) -> Single<PostModel> {
        return authProvider.rx.request(MultiTarget(HomeAPIs.getPost(id: id)))
            .tryMap(PostModel.self)
    }
    
    // 게시글 저장 유무 불러오기
    func checkPostSaved(id: Int) -> Single<Bool> {
        return authProvider.rx.request(MultiTarget(HomeAPIs.checkPostSaved(id: id)))
            .tryMap(Bool.self)
    }
    
    // 게시글 댓글 개수 불러오기
    func getPostCommentsCount(id: Int) -> Single<CommentCountModel> {
        return authProvider.rx.request(MultiTarget(HomeAPIs.getPostCommentsCount(id: id)))
            .tryMap(CommentCountModel.self)
    }
    
    // 게시글 댓글 불러오기
    func getPostComments(id: Int) -> Single<[CommentModel]> {
        return authProvider.rx.request(MultiTarget(HomeAPIs.getPostComments(id: id)))
            .tryMap([CommentModel].self)
    }
    
    // 게시글 저장
    func savePost(id: Int) -> Single<PlainResponseModel> {
        return authProvider.rx.request(MultiTarget(HomeAPIs.savePost(id: id)))
            .tryMap(PlainResponseModel.self)
    }
    
    // 게시글 저장 취소
    func cancelSavePost(id: Int) -> Single<PlainResponseModel> {
        return authProvider.rx.request(MultiTarget(HomeAPIs.cancelSavePost(id: id)))
            .tryMap(PlainResponseModel.self)
    }
    
    // 게시글 삭제
    func deletePost(id: Int) -> Single<PlainResponseModel> {
        return authProvider.rx.request(MultiTarget(HomeAPIs.deletePost(id: id)))
            .tryMap(PlainResponseModel.self)
    }
    
    // 댓글 생성
    func createPostComment(postId: Int, content: String) -> Single<PlainResponseModel> {
        return authProvider.rx.request(MultiTarget(HomeAPIs.createPostComment(postId: postId, content: content)))
            .tryMap(PlainResponseModel.self)
    }
    
    // 댓글 수정
    func patchPostComment(id: Int, content: String) -> Single<PlainResponseModel> {
        return authProvider.rx.request(MultiTarget(HomeAPIs.patchPostComment(id: id, content: content)))
            .tryMap(PlainResponseModel.self)
    }
    
    // 댓글 삭제
    func deletePostComment(id: Int) -> Single<PlainResponseModel> {
        return authProvider.rx.request(MultiTarget(HomeAPIs.deletePostComment(id: id)))
            .tryMap(PlainResponseModel.self)
    }
    
    // 채팅방 id 조회
    func createChatRoomId(userId: Int, groupPostId: Int) -> Single<CreateChatRoomResponseModel> {
        return authProvider.rx.request(MultiTarget(HomeAPIs.createChatRoomId(userId: userId, groupPostId: groupPostId)))
            .tryMap(CreateChatRoomResponseModel.self)
    }
    
    // MARK: - 내 프로필
    func getMyProfile(tab: String, page: Int, size: Int) -> Single<MyProfileResponseModel> {
        return authProvider.rx.request(MultiTarget(HomeAPIs.getMyProfile(tab: tab, page: page, size: size)))
