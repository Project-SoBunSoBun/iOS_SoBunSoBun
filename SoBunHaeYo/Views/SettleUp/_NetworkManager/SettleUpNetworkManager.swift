...
class SettleUpNetworkManager {
    ...
    func mySettleUps(status: String, page: Int, size: Int) -> Single<SettleUpResponseModel> {
        return authProvider.rx.request(MultiTarget(SettleUpAPIs.mySettleUps(status: status, page: page, size: size)))
            .tryMap(SettleUpResponseModel.self)
    }
    
    // 정산 완료 등록 메서드
    func putSettlementComplete(model: SettleUp3rdStepDataModel) -> Single<PlainResponseModel> {
        return authProvider.rx.request(MultiTarget(SettleUpAPIs.putSettlementComplete(model: model)))
            .tryMap(PlainResponseModel.self)
    }
    
    // 정산 상세 조회 메서드
    func getSettlement(settlementId: Int) -> Single<SettlementResponseModel> {
        return authProvider.rx.request(MultiTarget(SettleUpAPIs.getSettlement(settlementId: settlementId)))
