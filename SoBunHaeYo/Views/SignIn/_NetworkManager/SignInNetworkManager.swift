...
class SignInNetworkManager {
    ...
    func fetchAuthLoginKakao(accessToken: String) -> Single<AuthResponse> {
        return provider.rx.request(MultiTarget(SignInAPIs.authLoginKakao(accessToken: accessToken)))
            .tryMap(AuthResponse.self)
    }
    
    // 애플 로그인
    func fethAuthLoginApple(code: String, idToken: String) -> Single<AuthResponse> {
        return provider.rx.request(MultiTarget(SignInAPIs.authLoginApple(code: code, idToken: idToken)))
            .tryMap(AuthResponse.self)
    }
    
    // 애플 계정 연결 해제
    func fetchAuthRevokeApple() -> Single<PlainResponseModel> {
        return authProvider.rx.request(MultiTarget(SignInAPIs.authRevokeApple))
    ...
    func fetchAuthCompleteSignUp(loginToken: String, serviceTermsAgreed: Bool, privacyPolicyAgreed: Bool, marketingOptionalAgreed: Bool) -> Single<UserModel> {
        return provider.rx.request(MultiTarget(SignInAPIs.authCompleteSignUp(loginToken: loginToken, serviceTermsAgreed: serviceTermsAgreed, privacyPolicyAgreed: privacyPolicyAgreed, marketingOptionalAgreed: marketingOptionalAgreed)))
            .tryMap(UserModel.self)
    }
    
    // 서버에서 닉네임 중복 여부를 확인하는 메서드
    func checkNickname(nickname: String) -> Single<CheckNicknameModel> {
        return provider.rx.request(MultiTarget(SignInAPIs.checkNickname(nickname: nickname)))
    ...
    func saveProfile(nickname: String, profileImage: UIImage?) -> Single<PlainResponseModel> {
        ...
        return authProvider.rx.request(MultiTarget(SignInAPIs.saveProfile(nickname: nickname, profileImage: imageData)))
            .tryMap(PlainResponseModel.self)
    }
}
